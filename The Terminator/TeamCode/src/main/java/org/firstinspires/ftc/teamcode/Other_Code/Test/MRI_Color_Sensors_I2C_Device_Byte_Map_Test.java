...
@Disabled
public class MRI_Color_Sensors_I2C_Device_Byte_Map_Test extends OpMode {
    ...
    public void loop() {
        ...
        if (!touchState && gamepad1.guide) {  //If the touch sensor is just now being pressed (was not pressed last time through the loop but now is)
            ...
            if(LEDState){
                double lightA = getLightA();
                double lightC = getLightC();
                telemetry.addData("color A", lightA);
                telemetry.addData("color C", lightC);
                colorAreader.write8(3, 0);    //Set the mode of the color sensor using LEDState
                colorCreader.write8(3, 0);    //Set the mode of the color sensor using LEDState
                ...
                double lightA = getLightA();
                double lightC = getLightC();
                telemetry.addData("colors", lightA);
                telemetry.addData("color C", lightC);
                colorAreader.write8(3, 1);    //Set the mode of the color sensor using LEDState
                colorCreader.write8(3, 1);    //Set the mode of the color sensor using LEDState
            }
        ...
        if (!gamepad1.guide) {                //If the touch sensor is now pressed
            touchState = false;                  //Set the touchState to false to indicate that the touch sensor was released
        }
    }

    private double getLightA(){
        double colorA = 0;

        colorAcache = colorAreader.read(0x04, 1);

        byte bytecolorAcache = colorAcache[0];

        colorA = bytecolorAcache;

        return colorA;
    }

    private double getLightC(){
        double colorC = 0;

        colorCcache = colorCreader.read(0x04, 1);

        byte bytecolorCcache = colorCcache[0];

        colorC = bytecolorCcache;
